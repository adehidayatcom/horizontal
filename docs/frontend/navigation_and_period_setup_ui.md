...
Admin shell harus menentukan state periode sebelum render menu utama. **Wajib menyediakan "Global Context Switcher" (Pemilih Periode) di Top Navbar (Header)** agar admin bisa berpindah konteks ruang waktu (periode) kapan saja tanpa tersesat.
...
- **wajib mengubah warna tema header (misal: abu-abu gelap) sebagai isyarat visual (Visual Cue)**;
- tampilkan banner/badge peringatan permanen 🔒 `Mode laporan historis (Read-Only)`;
...
|---|---|---|---|
| Dashboard | `/admin/dashboard` | AKTIF / laporan historis | Wajib menampilkan komponen *Countdown* target Lebaran |
...
| 4 | Paket & BOM (Kloning) | Konfirmasi kloning masal paket dari tahun lalu (Status `Draft`) |
...
## 6.1 Manajemen Katalog Paket (Spreadsheet Mode)

Pengeditan detail ratusan paket TIDAK BOLEH dilakukan di dalam form konvensional atau Wizard. Halaman `/admin/master/paket` harus mengimplementasikan antarmuka skala Enterprise:

- **Spreadsheet Data Grid:** Menggunakan tabel *inline-editing* (seperti `MUI X Data Grid` atau sejenisnya) agar admin bisa mengetik harga dan mengubah item langsung di dalam tabel tanpa pop-up berlebih (mencegah *Click Fatigue*).
- **Tree-View Sidebar:** Ratusan paket wajib dikelompokkan ke dalam navigasi pohon (Kategori > Subkategori) di sebelah kiri tabel untuk memecah beban kognitif.
- **Mass Replace:** Wajib menyediakan fitur "Ganti Masal" (contoh: ganti Minyak A ke Minyak B di 150 paket sekaligus).
- **Traffic Lights Indicator:** Status visual per paket (🔴 Merah jika HPP melebihi Harga Jual, 🟡 Kuning jika masih Draft hasil kloning, 🟢 Hijau jika Siap Jual).
